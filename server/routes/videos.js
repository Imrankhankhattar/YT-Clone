...
router.post("/upload", async (req, res) => {
  ...
  try {
    console.log("Upload request received:", uploadInfo);
    console.log("User ID:", userId);
    
    const { filename: videoFilename, thumbnailFilename } = uploadInfo;
    
    console.log(`Processing upload for video: ${videoFilename}, thumbnail: ${thumbnailFilename}`);
    
    // Check if files exist locally
    const videoExists = await fileProcess.checkFileExists({
      type: "video",
      filename: videoFilename,
    });
    const thumbnailExists = await fileProcess.checkFileExists({
      type: "thumbnail",
      filename: thumbnailFilename,
    });
    
    if (!videoExists || !thumbnailExists) {
      throw new Error(`Files not found: video=${videoExists}, thumbnail=${thumbnailExists}`);
    }
    ...
    console.log(`Files stored - Video ID: ${videoStoreId}, Thumbnail ID: ${thumbnailStoreId}`);
    
    const { duration } = await fileProcess.videoInfo(uploadInfo.filename);
    uploadInfo.duration = duration;
    
    // Use userId from authentication if available, otherwise use uploader from request
    const uploaderId = userId || uploadInfo.uploader;
    
    console.log(`Creating video with uploader: ${uploaderId}`);
    ...
    console.log(`Video uploaded successfully: ${uploadInfo.title}, ID: ${video._id}`);
    res.json({
      success: true,
      videoId: video._id,
    });
      ...
      async (isAuth) => {
        ...
        if (isAuth && isExist) {
          ...
          try {
            ...
            console.log(`Error getting video store file: ${err.message}`);
      ...
      async (isAuth) => {
        ...
        if (isAuth && isExist) {
          ...
          try {
            ...
            console.log(`Error getting store file: ${err.message}`);
            // Continue without the file
          }
          shouldSend = true;
        }
